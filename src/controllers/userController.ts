...
class UserController {
  ...
  async getUserExams(req: Request, res: Response) {
    const { userId } = req.params
    
    try {
      const user = await User.findOne({ where: { id: userId }, relations: ['exams'] })
  
      if (!user) { return res.status(404).json('User not found') }
  
      return res.status(200).json(user.exams)
    } catch (err) {
      if (err.code === '22P02') { return res.status(404).json('Invalid user id') }
      console.error(err.message)
      return res.status(500).json('Server couldn\'t get user exams')
    }
  }
